...
func setupV1Routes(r *gin.Engine) {
	...
	admin := r.Group("/admin")
	{
		...
		admin.POST("/gifts/batch", batchCreateGifts)
...
func setupAvatarRoutes(avatars *gin.Engine) {
	...
	av2 := avatars.Group("/v2/avatars")
	av2.Use(v2ParamsToQuery())
	{
		av2.POST("/upload/pfp", uploadPfpHandler)
		av2.POST("/upload/banner", uploadBannerHandler)
		av2.POST("/overlays/reload", requiresAuth, requirePermission(PermManageSettings), reloadOverlays)
		av2.GET("/:username", avatarHandler)
		av2.HEAD("/:username", avatarHandler)
		av2.GET("/:username/overlay", overlayHandler)
		av2.HEAD("/:username/overlay", overlayHandler)
		av2.GET("/:username/banner", bannerHandler)
		av2.HEAD("/:username/banner", bannerHandler)
	}
}

func startAvatarServer() {
	go func() {
		avatars := gin.Default()
		setupAvatarRoutes(avatars)
